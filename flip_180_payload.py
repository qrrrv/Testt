...
class Plugin(BasePlugin):
    ...
    def create_settings(self):
        return [
            ...
            Input(
                ...
                text="Команда в чате",
                default=".flip",
                subtext="Например: .flip, .rotate или .reverse",
            ),
            Input(
                key="intro_delay_ms",
    ...
    def _command_prefix(self):
        prefix = safe_text(self.get_setting("chat_command", ".flip"))
        return prefix or ".flip"
    ...
    def _is_flipped(self, activity):
        if activity is None:
            return False
        try:
            current = safe_int(activity.getRequestedOrientation(), ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED)
            return current == self._target_orientation(activity)
        except Exception:
            return False
    ...
    def _handle_chat_command(self, message):
        text = safe_text(message)
        prefix = self._command_prefix()
        lower_text = text.lower()
        lower_prefix = prefix.lower()
        if lower_text != lower_prefix and not lower_text.startswith(lower_prefix + " "):
            return False

        remainder = safe_text(text[len(prefix):]).lower()

        def _run():
            activity = self._current_activity()
            if activity is None:
                return
            if remainder in ("", "toggle", "switch"):
                if self._is_flipped(activity):
                    self._restore_current(animate=False)
                else:
                    self._start_intro_sequence(force=True)
                return
            if remainder in ("off", "normal", "back", "restore", "reset"):
                self._restore_current(animate=False)
                return
            if remainder in ("on", "intro", "start"):
                self._start_intro_sequence(force=True)
                return
            if remainder in ("status", "state"):
                if self._is_flipped(activity):
                    log("[Flip180] chat command status: flipped")
                else:
                    log("[Flip180] chat command status: normal")
    ...
    def on_send_message_hook(self, _account, params):
        ...
        if not isinstance(text, str):
            return HookResult()
        try:
            if self._handle_chat_command(text):
                return HookResult(strategy=HookStrategy.CANCEL, params=params)
        except Exception as e:
            log(f"[Flip180] chat command error: {e}")
        return HookResult()

    def on_send_message(self, params):
        return self.on_send_message_hook(None, params)
